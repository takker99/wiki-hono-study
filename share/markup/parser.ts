...
export function toNodes(obj) {
  if (obj instanceof Array) {
    ...
    if (invalid) {
      throw new Error(`found invalid node - ${JSON.stringify(invalid)}`);
    }
  ...
  throw new Error(
    `invalid source, cannot convert to Nodes. - ${JSON.stringify(obj)}`,
  );
}

export function disableRegExpCapture(regexp, { replace, flags } = {}) {
  if (!(regexp instanceof RegExp)) {
    throw new Error("Invalid argument: not RegExp");
  }
...
export function createTextNode(value) {
  if (typeof value !== "string") {
    throw new Error("Invalid argument: not String");
  }
