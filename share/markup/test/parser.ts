...
import { disableRegExpCapture, flatten, Parser, toNodes } from "../parser";
...
describe("Parser - wiki syntax parser", function () {
  describe("utils", function () {
    describe("disableRegExpCapture", function () {
      it('should return new RegExp with replacing captures to "non-captureing parentheses"', function () {
        assert.regExpEqual(
          disableRegExpCapture(/[(hello)] (world|work)/),
          /[(?:hello)] (?:world|work)/,
          "disable all captures",
        );
        assert.regExpEqual(
          disableRegExpCapture(/[[(hello)]]/, { replace: (str) => `(${str})` }),
          /([[(?:hello)]])/,
          '"replace" option',
        );
        assert.regExpEqual(
          disableRegExpCapture(/[(image)\.(?:jpg|gif|png)]/),
          /[(?:image)\.(?:jpg|gif|png)]/,
  ...
  describe("replacers", function () {
    ...
    describe("wiki-title-link", function () {
      it("should parse [wiki::title]", function () {
        const nodes = Parser.wikiTitleLink(
          toNodes("hello [shokai::test] world"),
        );
        assert.deepEqual(nodes, [
          { type: "text", value: "hello " },
          {
            type: "wiki-title-link",
            wiki: "shokai",
            title: "test",
            source: "[shokai::test]",
          },
    ...
    describe("external-link-with-image", function () {
      it("should parse [http://example.com http://example.com/image.jpg]", function () {
        const nodes = Parser.externalLinkWithImage(
          toNodes(
            "hello [http://shokai.org https://gyazo.com/0ceeecc3c7d42e94a080e21f6a23f190.gif] world",
          ),
        );
        assert.deepEqual(nodes, [
          { type: "text", value: "hello " },
          {
            type: "external-link-with-image",
            link: "http://shokai.org",
            image: "https://gyazo.com/0ceeecc3c7d42e94a080e21f6a23f190.gif",
            source:
              "[http://shokai.org https://gyazo.com/0ceeecc3c7d42e94a080e21f6a23f190.gif]",
          },
          { type: "text", value: " world" },
        ]);
      });
    ...
    describe("external-link-with-image-reverse", function () {
      it("should parse [http://example.com/image.jpg http://example.com]", function () {
        const nodes = Parser.externalLinkWithImageReverse(
          toNodes(
            "hello [https://gyazo.com/0ceeecc3c7d42e94a080e21f6a23f190.gif http://shokai.org] world",
          ),
        );
        assert.deepEqual(nodes, [
          { type: "text", value: "hello " },
          {
            ...
            image: "https://gyazo.com/0ceeecc3c7d42e94a080e21f6a23f190.gif",
            source:
              "[https://gyazo.com/0ceeecc3c7d42e94a080e21f6a23f190.gif http://shokai.org]",
          },
          { type: "text", value: " world" },
        ]);
      });
    ...
    describe("external-link-with-description", function () {
      it("should parse [http://example.com example site]", function () {
        const nodes = Parser.externalLinkWithDescription(
          toNodes("hello [http://example.com example site] world"),
        );
        assert.deepEqual(nodes, [
          { type: "text", value: "hello " },
          {
            type: "external-link-with-description",
    ...
    describe("external-link-with-description-reverse", function () {
      it("should parse [example site http://example.com]", function () {
        const nodes = Parser.externalLinkWithDescriptionReverse(
          toNodes("hello [example site http://example.com] world"),
        );
        assert.deepEqual(nodes, [
          { type: "text", value: "hello " },
          {
            ...
            link: "http://example.com",
            description: "example site",
            source: "[example site http://example.com]",
          },
          { type: "text", value: " world" },
        ]);
      });
    ...
    describe("external-link", function () {
      it("should parse [http://example.com]", function () {
        const nodes = Parser.externalLink(
          toNodes("hello [http://shokai.org] world"),
        );
        assert.deepEqual(nodes, [
          { type: "text", value: "hello " },
          {
    ...
    describe("external-link-naked", function () {
      it("should parse http://example.com", function () {
        const nodes = Parser.externalLinkNaked(
          toNodes("hello http://shokai.org world"),
        );
        assert.deepEqual(nodes, [
          { type: "text", value: "hello" },
    ...
    describe("image", function () {
      it("should parse [http://example.com/image.gif]", function () {
        const nodes = Parser.image(
          toNodes("hello [http://example.com/image.gif] world"),
        );
        assert.deepEqual(nodes, [
          { type: "text", value: "hello " },
          {
            ...
            source: "[http://example.com/image.gif]",
          },
          { type: "text", value: " world" },
        ]);
      });
    ...
    describe("inline-code", function () {
      ...
      it("allow backquote in `inline code`", function () {
        const nodes = Parser.inlineCode(
          toNodes("hello `inline \\`code\\`` world"),
        );
        assert.deepEqual(nodes, [
          { type: "text", value: "hello " },
          {
  ...
  describe("parse", function () {
    it("should parse link", function () {
      assert.deepEqual(
        Parser.parse(
          "hello [[world]] [shokai][http://shokai.org] [general::test] [ざんまい::] かずすけ [http://shokai.org https://gyazo.com/0ceeecc3c7d42e94a080e21f6a23f190.gif][http://example.com example site][http://example.com/image.PNG]",
        ),
        [
          { type: "text", value: "hello " },
