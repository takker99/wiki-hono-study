...
import { validateRoute, validateTitle, validateWiki } from "../../share/route";
...
pageSchema.statics.findPagesByWiki = function (wiki) {
  return Page.findNotEmpty({ wiki }, "title image", {
    sort: { updatedAt: -1 },
  });
};
...
pageSchema.methods.findReverseLinkedPages = function (
...
pageSchema.methods.findInnerLinkedPages = async function (
  selector = "title image",
) {
...
pageSchema.methods.findRelatedPages = async function () {
  const [relateds, innerLinks] = await Promise.all([
    this.findReverseLinkedPages(),
    this.findInnerLinkedPages(),
  ]);
