...
import { ignoreFavicon, setUserContext } from "./middleware";
...
import { buildPath, defaultRoute, parseRoute } from "../../share/route";
...
router.get("/*", async (ctx, next) => {
  ...
  if (ctx.user) {
    ...
    const pagelist = (await Page.findPagesByWiki(wiki)).map((page) =>
      page.toHash()
    );
    const relatedPagelist = (await page.findRelatedPages()).map((page) =>
      page.toHash()
    );
