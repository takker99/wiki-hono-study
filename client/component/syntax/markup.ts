    ...
    return nodes.map((node) => {
      switch (node.type) {
        case "title-link": {
          ...
          return (
            <RouteLink action={action} route={{ wiki, title }}>
              {title}
            </RouteLink>
          );
        }
        ...
        case "wiki-link": {
          ...
          return (
            <RouteLink action={action} route={{ wiki }}>{wiki}::</RouteLink>
          );
        }
        case "wiki-title-link": {
          let { wiki, title } = node;
          if (validateRoute({ wiki, title }).invalid) {
            return <span>{node.source}</span>;
          }
          return (
            <RouteLink action={action} route={{ wiki, title }}>
              {`${wiki}::${title}`}
